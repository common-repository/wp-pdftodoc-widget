<?php
/*
Plugin Name: WP PDFTOWORD Converter Widget
Plugin URI: http://www.investintech.com/resources/blog/archives/204-WP-PDFTOWORD-Converter-Widget-Plugin.html
Description: Integrate PDF to WORD converter into any worpress website
Version: 0.1
Author: investintech
Author URI: http://www.investintech.com
Usage: View readme.txt

Copyright (C) <2010>  <WP PDFTOWORD Converter Widget>
...
*/
load_plugin_textdomain('wp-pdftodoc-widget', false, dirname(plugin_basename(__FILE__)) . '/lang');
define('WPDFTODOCW_PLUGIN_DIR',dirname(__FILE__));         //Abs path to the plugin directory
define('WPDFTODOCW_PLUGIN_URL','/wp-content/plugins/wp-pdftodoc-widget');  
define('WPDFTODOCW_CONVERSION_TYPE','PDF2MSWordStandard');
define('WPDFTODOCW_APS_WEBSERVICE_DOMAIN','http://184.72.226.232');
define('WPDFTODOCW_APS_WEBSERVICE_URL',WPDFTODOCW_APS_WEBSERVICE_DOMAIN.'/APSWebService/APSWebService.asmx?WSDL');


if (!class_exists('Wpdftodocw')) {
	...
	class Wpdftodocw {
        ...
        function Wpdftodocw() {
         	...
         	$this->wholeDir =  WPDFTODOCW_PLUGIN_DIR. '/files/'.$this->unique.'/';
         	$this->client = new SoapClient(WPDFTODOCW_APS_WEBSERVICE_URL,array('cache_wsdl' => WSDL_CACHE_NONE, 'compression' => SOAP_COMPRESSION_ACCEPT | SOAP_COMPRESSION_GZIP));
         ...
         */
		function add_crons()  {
			wp_schedule_event(time(), 'daily', 'wpdftodocw_cleanup');
        }
        /*
         * Removes cron action from WP crontab
         */
        function remove_crons()  {
        	wp_clear_scheduled_hook('wpdftodocw_cleanup');
        }
        /*
		...
		function do_cleanup()  {
        	...
        	$skindir = WPDFTODOCW_PLUGIN_DIR.'/files/';
        ...
        */
        function get_options() {
			...
			$o = get_option('wp_pdftodoc_widget');
		...
		function init_options() {
			$o = array(
				0 => array(
				'type' => WPDFTODOCW_CONVERSION_TYPE,
				'max_size' => 1,
				'affid' => '',
				'afflink' => 1
				), 
			);
			update_option('wp_pdftodoc_widget', $o);		
			return $o;
		}
		/*
		...
		function meta_boxes() {
        	if ( current_user_can('unfiltered_html') ) {
        		// settings page
        		add_options_page("WP PDF to WORD Converter Widget", "WP PDF to WORD Converter Widget", 'manage_options', "wp-pdftodoc-widget", array('WpdftodocwAdmin', 'make_admin_page'));
			}
        }
        /*
        ...
        function add_mootools(&$scripts) {
        	if (!isset($twobuy_mm_object)) {
        		...
        		$scripts->add( 'mootools-core', WPDFTODOCW_PLUGIN_URL.'/js/mootools-core-1.3.js', false, '1.3' );
				$scripts->add( 'mootools-more', WPDFTODOCW_PLUGIN_URL.'/js/mootools-more-1.3.js', array('mootools-core'), '1.3' );
				$scripts->add( 'swift-uploader', WPDFTODOCW_PLUGIN_URL.'/js/Swiff.Uploader.js', array('mootools-more'), '3.0' );
        	}
        } 
		...
		function init_scripts($posts){
			...
			if (is_active_widget(false, false, 'wpdftodocw', true)){
		...
		function create_rewrite_rules($rewrite) {
			$newrules = array();
			$newrules['wpdftodocw-upload/(.*)$'] = 'index.php?wpdftodocw-upload=$matches[1]';
			$newrules['wpdftodocw-download/(.*)$'] = 'index.php?wpdftodocw-download=$matches[1]';
			return $newrules + $rewrite;
        }
        
	 	function add_query_var($vars) {
        	array_push($vars,'wpdftodocw-upload');
        	array_push($vars,'wpdftodocw-upload-type');
        	array_push($vars,'wpdftodocw-download');
			return($vars);
        }
		...
		function is_wpc_template() {
    		...
    		$keyword = ( isset($wp_version) && ($wp_version >= 2.0) ) ? 
                get_query_var('wpdftodocw-upload') : 
                $GLOBALS['wpdftodocw-upload'];
		...
		function is_wpc_template_download() {
    		global $wp_version;
    		$keyword = ( isset($wp_version) && ($wp_version >= 2.0) ) ? 
                get_query_var('wpdftodocw-download') : 
                $GLOBALS['wpdftodocw-download'];
		...
		function template_redirect_upload() {
        	global $wp_the_query;
        	$args = array(
        			'max_size' => $wp_the_query->query_vars["wpdftodocw-upload"],
        			'type' => $wp_the_query->query_vars["wpdftodocw-upload-type"]
        			);

        	include WPDFTODOCW_PLUGIN_DIR . '/wp-pdftodoc-widget-upload.php';
			die;
        }
		...
		function template_redirect_download() {
        	global $wp_the_query;
        	$args = array(
        			'path'=>$wp_the_query->query_vars["wpdftodocw-download"]
        			);

        	include WPDFTODOCW_PLUGIN_DIR . '/wp-pdftodoc-widget-download.php';
			die;
        }
        /*
		 ...
		 */
         function process_upload($args) {
			...
			ini_set('default_socket_timeout',480);
         	...
         	$this->inputFileFolderURL =  $this->site_http_root . WPDFTODOCW_PLUGIN_URL. '/files/'.$this->unique.'/';
        ...
        function process_download($args) {
        	...
        	$file = file_get_contents(WPDFTODOCW_APS_WEBSERVICE_DOMAIN . $link);
		...
		function getSkins() {
			$ret = array();
         	$skindir = WPDFTODOCW_PLUGIN_DIR.'/skins/';
         	$inside = scandir($skindir);
         	foreach($inside as $in) {
         		if (is_dir($skindir.$in) && $in != '.' && $in != '..') {
         			$ret[] = $in;
...
if (class_exists("Wpdftodocw")) {
	$wpdftodocw_object = new Wpdftodocw();
}
//Actions and Filters	
if (isset($wpdftodocw_object)) {
	/*
	 * Cleanup crons related
	 */
	register_activation_hook(__FILE__,array(&$wpdftodocw_object, 'add_crons'));
	register_deactivation_hook(__FILE__, array(&$wpdftodocw_object, 'remove_crons'));
	add_action('wpdftodocw_cleanup', array(&$wpdftodocw_object, 'do_cleanup'));
	/*
	 * Admin related
	 */
	function wp_pdftodoc_widget_admin() {
		include WPDFTODOCW_PLUGIN_DIR . '/wp-pdftodoc-widget-admin.php';
	}
	foreach ( array('page.php', 'post-new.php', 'post.php', 'settings_page_wp-pdftodoc-widget') as $hook ) {
		add_action("load-$hook", 'wp_pdftodoc_widget_admin');
	}
	add_action('admin_menu', array(&$wpdftodocw_object, 'meta_boxes'), 30);	
	//add_shortcode('wpdftodoc', array(&$wpdftodocw_object,'shortcode'));
	/*
	 * Front End related
	 */
	add_action('wp_default_scripts', array(&$wpdftodocw_object, 'add_mootools'), 0);
	
	add_filter('the_posts', array(&$wpdftodocw_object, 'init_scripts'));
	add_filter('rewrite_rules_array', array(&$wpdftodocw_object,'create_rewrite_rules'));
	add_filter('query_vars', array(&$wpdftodocw_object, 'add_query_var'));
	add_filter('init', array(&$wpdftodocw_object, 'flush_rules'));
	add_action('parse_query', array(&$wpdftodocw_object, 'parse_query'));
	/*
	 * Widgets
	 */
	include_once(WPDFTODOCW_PLUGIN_DIR.'/wp-pdftodoc-widget-widget.php');
} 
?>
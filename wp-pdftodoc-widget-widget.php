...
class Wpdftodocw_Widget extends WP_Widget {
	public $site_http_root;
	public $input = array();
	
	function Wpdftodocw_Widget() {
		$widget_ops = array('classname' => 'widget_wpdftodocw', 'description' => __('Add PDFTODOC Converter into sidebar'));
		$control_ops = array('width' => 400, 'height' => 350);
		$this->WP_Widget('wpdftodocw', __('WP PDFTOWORD Converter Widget'), $widget_ops, $control_ops);
		$this->site_http_root = get_option('siteurl');
	}

	function widgets_init() {
		register_widget('Wpdftodocw_Widget');
	} # widgets_init()
	...
	function getWidgetStyle( $input ) {
		$this->input = $input;
		$script = '';
		if (file_exists(WPDFTODOCW_PLUGIN_DIR . '/skins/' . $input['skin'] . '/skin.css')) {
			$cscript = file_get_contents(WPDFTODOCW_PLUGIN_DIR . '/skins/' . $input['skin'] . '/skin.css');
	...
	function getWidgetScript( $max_size, $type, $proot ) {		
		...
		// Uploader instance
		var swf".md5($this->get_field_id('swf'))." = new Swiff.Uploader('$proot/wp-content/plugins/wp-pdftodoc-widget/js/Swiff.Uploader.swf', {
			url: '$proot/?wpdftodocw-upload=$msize&wpdftodocw-upload-type=$type',
			verbose: false,
			queued: 1,
			multiple: false,
				...
				if (resp.status == 0) {
					...
					if (Browser.ie){
						var getAnchor = new Element('a', {
							'href': '$proot/?wpdftodocw-download=' + encode64(resp.output),
							'class': 'ieDownload',
							'html': 'Download'
						});
						getAnchor.inject(note);
					} else {
						top.location.href = '$proot/?wpdftodocw-download=' + encode64(resp.output);
					}
				}
...
";
	...
	function widget( $args, $instance ) {
		...
		global $wpdftodocw_object;
		$o = $wpdftodocw_object->get_options();
		
		extract($args);
		$title = apply_filters('widget_title', empty($instance['title']) ? '' : $instance['title']);
		$type = WPDFTODOCW_CONVERSION_TYPE;
		$skin = apply_filters( 'widget_wpdftodocw', $instance['skin'] );
		$max_size = apply_filters( 'widget_wpdftodocw', !$instance['max_size'] ? $o[0]['max_size'] : $instance['max_size'] );
		...
		'.$this->getWidgetStyle(array(
			'skin' => $skin,
			'type' => $type,
			'site_subfolder' => $wpdftodocw_object->subfolder_path,
			'plugin_url' => WPDFTODOCW_PLUGIN_URL,
		...
		'. $this->getWidgetScript($max_size, $type, $wpdftodocw_object->subfolder_path) .'
		</script>
		<div class="wpdftodocw-content" id="'.$this->get_field_id('content').'">
			<div class="wpdftodocw-container" id="'.$this->get_field_id('container').'">
				<div class="wpdftodocw-note" id="note'.md5($this->get_field_id('note')).'">
					Please click the Browse button to upload your PDF 
				</div>
				<div class="wpdftodocw-browse-wrap" id="browse-wrap'.md5($this->get_field_id('browse-wrap')).'">
					<div class="wpdftodocw-browse" id="browse'.md5($this->get_field_id('browse')).'">
			...
			'<div class="guarantee">
				<a href="http://www.investintech.com/able2extract.html?invt='.$o[0]['affid'].'c142"><strong>Convert PDF to Word</strong></a>
			</div>'
			:
			''
			)
...
?>
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id('skin'); ?>" name="<?php echo $this->get_field_name('skin'); ?>">
				<?php
					foreach ($wpdftodocw_object->getSkins()as $sk) {
...
add_action('widgets_init', array('Wpdftodocw_Widget', 'widgets_init'));
?>